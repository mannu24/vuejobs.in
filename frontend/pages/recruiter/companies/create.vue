<template>
  <div class="max-w-2xl">
    <h1 class="text-xl font-bold text-gray-900 mb-6">Add Company</h1>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="form.website" type="url" class="w-full rounded-lg border-gray-300 text-sm" placeholder="https://...">
          </div>
          <div>
        ...
        <div>
          <label class="block text-xs font-medium text-gray-600 mb-1">About</label>
...
const form = reactive({ name: '', headline: '', website: '', logo_url: '', size: '', industry: '', about: '', is_public: true })

async function handleSubmit() {
  submitting.value = true; error.value = ''
  try { await apiFetch('/companies', { method: 'POST', body: form }); navigateTo('/recruiter/companies') }
  catch (e: any) { error.value = e?.data?.message || 'Failed to create company' }
